...
export function useFileManagerActions(options: UseFileManagerActionsOptions) {
  ...
  useEffect(() => {
    const unregister = authLifecycleRegistry.register({
      id: "file-manager-delete-confirm",
  ...
  }, [destroyDeleteConfirm, options.ownerId]);
  ...
  const createFolder = async () => {
    ...
    try {
      ...
      if (response.code !== 1 || !response.data?._id) {
        toast.error(response.message || "新建文件夹失败");
        return;
      }
      ...
      if (!isAccountScopeCurrent(scope)) return;
      toast.error(getErrorMessage(error, "新建文件夹失败"));
    } finally {
    ...
    const confirm = confirmDialog({
      ...
      onOk: async () => {
        ...
        try {
          ...
          if (missing > 0) toast.warning(`已删除 ${response.data.deletedCount} 项，${missing} 项未处理`);
          else toast.success(`已删除 ${response.data.deletedCount} 项`);
  ...
  const moveItems = async (
    ...
    try {
      ...
      if (failed.length || skipped.length) {
        toast.warning(`已移动 ${moved.length} 项，跳过 ${skipped.length} 项，失败 ${failed.length} 项`);
      } else toast.success(`已移动 ${moved.length} 项`);
      options.setSelectedIds([]);
