  ...
  return <Modal open={open} title={confirmEnd ? "确认结束所有人的会议？" : "离开会议"}
    ...
    <div className="space-y-4 py-4 text-sm text-text-primary">
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" disabled={busy} onClick={onLeave}>仅我离开</Button>
        {isHost && (confirmEnd
          ? <Button variant="destructive" loading={busy} onClick={() => void onEnd()}>确认结束所有人</Button>
          : <Button variant="destructive" onClick={() => setConfirmEnd(true)}>结束所有人…</Button>)}
        <Button variant="outline" disabled={busy} onClick={() => { setConfirmEnd(false); onCancel(); }}>取消</Button>
      </div>
    </div>
  </Modal>;
}
