...
export function ActiveMeetingRoom({ meetingTitle = "", meetingHostId = "", meetingStartTime, meetingAccessToken, onAccessRejected, media }: MeetingRoomProps & { media: LocalMedia }): ReactElement {
  ...
  const participants = useMeetingParticipants({ user, roomUsers: session.roomUsers, localPeerId: session.localPeerId,
    ...
    audioEnabled: media.audio.open, isMobile, onMobileSelect: panels.closeParticipants });
  const isHost = Boolean(user?.id && user.id === meetingHostId);
  const actions = useRoomActions({ roomId, media, session, isHost });
  return <>
    <MeetingRoomView localVideoRef={localVideoRef} participants={participants.participants} activeParticipantId={participants.activeParticipantId}
      ...
      endActionLabel="离开会议" ending={actions.ending} onCloseParticipants={panels.closeParticipants} onSelectParticipant={participants.selectParticipant}
      ...
      onSendComment={actions.sendComment} onToggleComment={panels.toggleComment} onToggleParticipants={panels.toggleParticipants} onEndMeeting={actions.openExit} />
