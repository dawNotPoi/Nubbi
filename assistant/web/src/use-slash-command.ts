 ...
 */
export const useSlashCommand = (input: UseSlashCommandInput) => {
  const {
    generating, onSend, onSelectModel, onToggleSkill, onToggleServer,
    models, skills, mcpServers,
  } = input;
  ...
  const handleKeyDown = (event: KeyboardEvent<HTMLTextAreaElement>): void => {
    ...
    const total = command === null
      ? commandMatches.length
      : command === "model" ? modelMatches.length
      : command === "skill" ? skillMatches.length
      : command === "mcp" ? mcpMatches.length
      : 0;
