...
export const SettingsContent = ({
  ...
  <>
    ...
    {section === "model" ? (
      <ModelSettingsPanel configAccessToken={configAccessToken} />
    ) : (
      <div className="space-y-3 p-3">
        <Button className="w-full" disabled={loading} onClick={onCreate} type="button">
          <Plus />
          新增 MCP Server
        </Button>
        ...
        {!servers.length && !loading ? (
          <p className="py-10 text-center text-sm text-muted-foreground">尚未配置 MCP Server</p>
        ) : null}
