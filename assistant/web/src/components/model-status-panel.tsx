 ...
 * @returns 面板视图。
 */
export const ModelStatusPanel = ({ info }: { info: ModelStatusInfo }) => {
  const { tokenUsage, contextUsage } = info;
  const contextPercent = contextUsage.maxTokens > 0
    ? Math.min(100, Math.round((contextUsage.usedTokens / contextUsage.maxTokens) * 100))
    : 0;
  const levelColor = contextPercent >= 90
    ? "text-red-500"
    : contextPercent >= 70
      ? "text-amber-500"
      : "text-emerald-500";
  ...
  return (
    <div className="py-1">
      ...
      <div className="py-1">
        ...
        <Row label="总计">{formatTokens(tokenUsage.totalTokens)} tokens</Row>
      </div>

      <SectionTitle icon={<Gauge className="size-3.5" />} text="上下文占用" />
