...
import type { McpDraft } from "./mcp-form-utils";
...
export const HttpFields = ({
  ...
  <>
    <label className="block space-y-1.5 text-sm font-medium">
      MCP HTTP URL
      <input className={inputClass} onChange={(event) => onChange({ ...draft, url: event.target.value })} placeholder="https://example.com/mcp" value={draft.url} />
    </label>
...
export const StdioFields = ({
  ...
  <>
    <label className="block space-y-1.5 text-sm font-medium">
      启动命令
      <input className={inputClass} onChange={(event) => onChange({ ...draft, command: event.target.value })} placeholder="node" value={draft.command} />
    </label>
    <label className="block space-y-1.5 text-sm font-medium">
      启动参数（每行一个）
      <textarea
    ...
    <label className="block space-y-1.5 text-sm font-medium">
      工作目录（可选）
      <input className={inputClass} onChange={(event) => onChange({ ...draft, cwd: event.target.value })} placeholder="缺省继承 Assistant 进程目录" value={draft.cwd} />
    </label>
