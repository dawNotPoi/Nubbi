...
import { mcpServerSchema, type McpServerConfig } from "../../integrations/mcp/mcp-config.ts";
...
export class McpConfigController {
  constructor(@Inject(McpConfigService) private readonly mcpConfig: McpConfigService) {}
   ...
   */
  @Put("servers/:id")
  async update(@Param("id") id: string, @Body() input: unknown): Promise<McpServerConfig> {
    try {
  ...
  @HttpCode(200)
  async setEnabled(@Param("id") id: string, @Body() input: unknown): Promise<McpServerConfig> {
  ...
  @Post("test")
  @HttpCode(200)
  async test(@Body() input: unknown): Promise<Awaited<ReturnType<McpConfigService["test"]>>> {
    try {
