...
const idSchema = z.string().regex(/^[a-z0-9-]+$/, "ID 只能包含小写字母、数字和连字符");
const stringMapSchema = z.record(z.string()).default({});

/** MCP 配置校验，按 HTTP 与 stdio 传输方式约束必要字段。 */
...
export const listMcpServers = async (): Promise<McpServerConfig[]> => {
  ...
  if (!source) return [];
  const parsed = z.object({ servers: z.array(z.unknown()).default([]) }).parse(JSON.parse(source));
 ...
 */
const mutateServers = async <T>(operation: (servers: McpServerConfig[]) => Promise<T>): Promise<T> => {
 ...
 */
export const createMcpServer = async (input: McpServerConfig): Promise<McpServerConfig> =>
  mutateServers(async (servers) => {
    const id =
      input.id ?? generateServerId(input.name, new Set(servers.flatMap((server) => (server.id ? [server.id] : []))));
 ...
 */
export const updateMcpServer = async (id: string, input: McpServerConfig): Promise<McpServerConfig> =>
  mutateServers(async (servers) => {
