...
import { createMcpServer, deleteMcpServer, listMcpServers, testMcpServer, updateMcpServer } from "../../api.ts";
import { Button, IconButton, Switch } from "../../components/controls.tsx";
import { colors } from "../../theme.ts";
import type { McpConnectionTest, McpServerConfig } from "../../types.ts";
import { McpForm } from "./mcp-form.tsx";
...
export const McpPanel = ({
  ...
  const load = useCallback(async (): Promise<void> => {
    setBusy(true);
    try {
      setServers(await listMcpServers(baseUrl, configAccessToken));
      setDanger(false);
    } catch (caught) {
      setDanger(true);
  ...
  const save = async (server: McpServerConfig): Promise<void> => {
    setBusy(true);
    try {
      if (editing) await updateMcpServer(baseUrl, configAccessToken, server);
      else await createMcpServer(baseUrl, configAccessToken, server);
      await load();
  ...
  const test = async (server: McpServerConfig): Promise<McpConnectionTest> => {
    setBusy(true);
    try {
      const result = await testMcpServer(baseUrl, configAccessToken, server);
      setDanger(false);
  ...
  const toggle = async (server: McpServerConfig): Promise<void> => {
    setBusy(true);
    try {
      await updateMcpServer(baseUrl, configAccessToken, { ...server, enabled: !server.enabled });
      await load();
    } catch (caught) {
      setDanger(true);
  ...
  const remove = (server: McpServerConfig): void => {
    Alert.alert("删除 MCP", `确认删除“${server.name}”？`, [
      { text: "取消", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: () =>
          void deleteMcpServer(baseUrl, configAccessToken, server.id ?? "")
            .then(load)
  ...
  return (
    <View style={styles.panel}>
      <Button loading={busy && !servers.length} onPress={() => setEditing(null)}>
        新增 HTTP MCP
      </Button>
      ...
      {servers.map((server) => (
        <View key={server.id ?? server.name} style={styles.serverItem}>
          ...
          <View style={styles.serverActions}>
            <IconButton
              icon={<FlaskConical color={colors.primary} size={18} />}
              label={`测试 ${server.name}`}
              onPress={() =>
