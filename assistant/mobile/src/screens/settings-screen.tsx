...
import { ModelPanel } from "./settings/model-panel";
    ...
    <SafeAreaView style={styles.safeArea}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.flex}>
        <View style={styles.modalHeader}>
          ...
          {token ? <IconButton icon={<KeyRound color={colors.text} size={20} />} label="锁定设置" onPress={() => void lock()} /> : null}
          <IconButton icon={<X color={colors.text} size={22} />} label="关闭" onPress={onClose} />
        </View>
        {token === null ? (
          <View style={styles.center}><Text style={styles.itemSubtitle}>正在读取安全配置</Text></View>
        ) : !token ? (
          <View style={styles.unlock}>
            <View style={styles.unlockIcon}><KeyRound color={colors.primary} size={26} /></View>
            <Field autoCapitalize="none" autoCorrect={false} label="设置管理密钥" onChangeText={setTokenInput} secureTextEntry value={tokenInput} />
            <Notice danger message={error} />
            <Button disabled={!tokenInput.trim()} loading={busy} onPress={() => void unlock()}>解锁配置</Button>
            <Button onPress={onChangeServer} tone="secondary">更换 Assistant API</Button>
          </View>
        ) : (
          <>
            <View style={styles.tabs}>
              {(["model", "mcp"] as const).map((item) => (
                <Pressable key={item} onPress={() => setTab(item)} style={[styles.tab, tab === item && styles.tabActive]}>
                  {item === "model" ? <Server color={tab === item ? colors.primary : colors.muted} size={17} /> : <FlaskConical color={tab === item ? colors.primary : colors.muted} size={17} />}
                  <Text style={[styles.tabText, tab === item && styles.tabTextActive]}>{item === "model" ? "模型" : "MCP"}</Text>
                </Pressable>
              ))}
            </View>
            <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
              {tab === "model" ? <ModelPanel baseUrl={baseUrl} token={token} /> : <McpPanel baseUrl={baseUrl} token={token} />}
              <Button onPress={onChangeServer} tone="secondary">更换 Assistant API</Button>
            </ScrollView>
          </>
        )}
