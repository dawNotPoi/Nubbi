...
- 需求、接口、交互与行为口径的唯一来源是 `docs/<module>/PRD.md` 及该模块的 plan / verification 文档；品牌与视觉方向以 `docs/ui/rebuild-plan.md` 为准。
- 维护流程固定为：先在方案文档定位并更新对应段落 → 再改代码 → 方案与代码放进同一 commit 并保持一致。
- 禁止只改代码不改方案；也不要用代码现状反向解释方案。代码与方案冲突时，先与用户确认口径，再同步两边。
- 方案里没有的口径不得为实现便利自行添加。实现中新增或放宽的决策（时间/长度校验、默认值、提示时机、错误码、层级、触控目标等）必须写回方案，不能只留在对话里。
- 讨论产生的决策按 `.agent/rules/discussion-log.md` 写入 `docs/<module>/discussion.md`；暂不处理的项写入对应方案文档的「已知遗留」段落，不在聊天记录里留待办。
- 维护对话以方案为准：用户描述需求时先映射到具体方案段落，改完后在回复中明确列出改了哪几段方案。
